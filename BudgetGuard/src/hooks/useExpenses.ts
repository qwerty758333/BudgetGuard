...
import { getUserExpenses } from '../utils/storage'
...
async function queryExpenses(activeUserId: string) {
  const base = supabase.from('expenses').select('*').eq('user_id', activeUserId)

  const byCreatedAt = await base.order('created_at', { ascending: false })
  if (!byCreatedAt.error) return byCreatedAt

  // Legacy tables may lack created_at (ordering triggers REST 500).
  return base.order('date', { ascending: false })
}

async function loadLocalExpenses(activeUserId: string): Promise<Expense[]> {
  const local = await getUserExpenses(activeUserId)
...
export function useExpenses(userId: string | undefined) {
  ...
  const fetchExpenses = useCallback(async () => {
    ...
    const local = await loadLocalExpenses(activeUserId)
    if (local.length > 0) {
      setExpenses(local)
    }

    const { data, error: fetchError } = await queryExpenses(activeUserId)
